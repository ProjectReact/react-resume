...
class NavMenu extends Component{
    ...
    render(){
        ...
        const { collapse, handleCollap, pathname } = this.props;

        return (
            <aside className={ collapse ? "layout-aside layout-aside-collapse" : "layout-aside" }>
                <div className="layout-logo">
                    <i className="iconfont icon-jianli logo"></i>
                    { collapse ? "" : <span className="title-text"> 梁成进 </span> }
                </div>
                <Menu mode="inline" theme="dark" defaultSelectedKeys={["home"]} selectedKeys={[pathname]}>
                    { this.getMenuItems( data )}
                </Menu>
                <div className={ collapse ? "aside-action aside-action-collapse" : "aside-action"} onClick={handleCollap}>
